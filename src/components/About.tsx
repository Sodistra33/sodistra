...
const About = () => {
  ...
  const values = ["Excellence et qualité garanties", "Plus de 17 ans d'expérience", "Équipe de professionnels qualifiés", "Respect des délais et budgets"];
  useEffect(() => {
    fetchContents();
  }, []);
  ...
  if (loading) {
    return <section className="py-20 bg-secondary flex items-center justify-center">
  ...
  return <section id="apropos" className="py-20 bg-secondary">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="animate-slide-up">
            ...
            {contents[1] && <p className="text-muted-foreground text-lg mb-8 leading-relaxed">{contents[1].description}</p>}
            <div className="space-y-4">
              {values.map((value, index) => <div key={index} className="flex items-center gap-3">
          ...
          <div className="relative animate-slide-up animate-delay-200">
            {contents.length > 0 ? <Carousel plugins={[Autoplay({
            delay: 2000
          })]} className="w-full">
                <CarouselContent>
                  {contents.flatMap(content => (content.images && content.images.length > 0 ? content.images : [content.image_path]).filter(Boolean).map((imageUrl, idx) => <CarouselItem key={`${content.id}-${idx}`}>
              ...
              </Carousel> : <div className="relative rounded-2xl overflow-hidden shadow-lg bg-primary/10 h-[500px] flex items-center justify-center">
